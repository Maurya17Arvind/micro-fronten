...
export class LoadProjects {
  async loadRemoteComponent(port: number, remoteName: string): Promise<any> {
    try {
      console.log(`Attempting to load ${remoteName} from http://localhost:${port}/remoteEntry.json`);
      ...
      console.log(`Component extracted for ${remoteName}:`, component.name || 'Anonymous Component');
      return component;
    } catch (error) {
